...
public:
    ...
    void check_finiteness(const te::dataframe& x_data,
        ...
        const auto finiteness_desc =
            roc_auc_score::descriptor<Float, Method>{}.set_allow_NaN(allowNaN);
...
using finiteness_types = COMBINE_TYPES((float, double), (roc_auc_score::method::dense));

TEMPLATE_LIST_TEST_M(finite_checker_batch_test,
                     "ROC AUC score typical",
...
TEMPLATE_LIST_TEST_M(finite_checker_batch_test,
                     "ROC AUC score compute one element matrix",
                     "[roc_auc_score][integration][batch]",
                     finiteness_types) {
    SKIP_IF(this->not_float64_friendly());
