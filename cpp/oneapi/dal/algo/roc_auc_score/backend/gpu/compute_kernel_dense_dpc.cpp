...
double compute_roc_auc_score(sycl::queue& queue,
                    ...
                    element_wise(queue, kernel_neq, base, offset, diff_offset, {allocenv}).wait_and_throw();
                    ...
                    queue.parallel_for(sycl::range<1>(total_ranks), [=](sycl::id<1> i) {
                        bk::atomic_global_add(ps_ptr + 2*diff_ptr[i] + static_cast<std::uint32_t>(y0_ptr[i]), Float(1));
                    }).wait_and_throw();
                    ...
                    Float* tps_ptr = tps.get_mutable_data();
                    Float* fps_ptr = fps.get_mutable_data();
