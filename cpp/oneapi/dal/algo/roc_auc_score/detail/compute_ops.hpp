...
struct compute_ops {
    ...
    void check_preconditions(const Descriptor& params, const input_t& input) const {
        ...
        if (!input.get_y0().has_data() || !input.get_y1().has_data()) {
...
#ifdef ONEDAL_DATA_PARALLEL
    template <typename Context>
    void operator()(const Context& ctx, const Descriptor& desc, const table& y0, const table& y1, double& res) {
        compute_ops_dispatcher<Context, float_t, method_t, task_t>()(ctx, desc, y0, y1, res);
    }
#endif
};
